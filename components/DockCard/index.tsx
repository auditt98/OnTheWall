...
export const DockCard = ({ children, className, text, textClassname }: DockCardProps) => {
  ...
  return (
    <div className={`dock-card-container ${className}`}>
      <animated.button
        ref={cardRef}
        className={'dock-card group'}
        onClick={handleClick}
        style={{
          width: size,
          height: size,
      ...
      >
        {children}
        <div className={`${textClassname} opacity-0 group-hover:opacity-100 flex transition-opacity duration-500 w-max text-center`}>{text}</div>
      </animated.button>
